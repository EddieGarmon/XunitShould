    ...
    public class AssertActualExpectedException : XunitException
    {
        ...
        public AssertActualExpectedException(object expected, object actual, string userMessage)
            ...
            if (actual != null && expected != null && Actual == Expected && actual.GetType() != expected.GetType()) {
                Actual += String.Format(CultureInfo.CurrentCulture,
                                        " ({0})",
                                        actual.GetType()
                                              .FullName);
                Expected += String.Format(CultureInfo.CurrentCulture,
                                          " ({0})",
                                          expected.GetType()
                                                  .FullName);
            }
        ...
        private static string ConvertToSimpleTypeName(TypeInfo typeInfo) {
            ...
            IEnumerable<string> simpleNames = typeInfo.GenericTypeArguments.Select(type => ConvertToSimpleTypeName(type.GetTypeInfo()));
            int backTickIdx = typeInfo.Name.IndexOf('`');
            if (backTickIdx < 0) {
